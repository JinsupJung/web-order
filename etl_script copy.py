...
def insert_mysql_ARTransactionsLedger(conn, df):
    ...
    try:
        cursor = conn.cursor()
        insert_query = """
            INSERT INTO ARTransactionsLedger_1 (
                transaction_date,
                representative_code,
                client,
                outlet_name,
        ...
        data = [
            (
                row['date'],
                row['rep_code'] if 'rep_code' in row else ' ',  # representative_code
                row['rechain_no'],
                row['full_name'],
                row['total'] if 'total' in row else 0.00,  # debit
                row['total'] if 'total' in row else 0.00   # food_material_sales
            )
...
def insert_mysql_AROrderDetailsItem(conn, df):
    """
    AROrderDetailsItem 테이블에 데이터 삽입
    """
    try:
        cursor = conn.cursor()
        insert_query = """
            INSERT INTO AROrderDetailsItem_1 (
                order_date,
                rep_code,
                rep_name,
                client_code,
        ...
        data = [
            (
                ...
                row['rep_full_name'] if 'rep_full_name' in row else None,
                row['rechain_no'],
                row['full_name'],
                row['item_no'],
                row['item_full_name'],
                ...
                row['item_price'] if 'item_price' in row else None,
                row['order_amount'] if 'order_amount' in row else None,
                row['item_tax'] if 'item_tax' in row else None,
                row['total'] if 'total' in row else None,
...
def insert_mysql_AROrderDetails_1(conn, df):
    """
    AROrderDetails_1 테이블에 데이터 삽입
    """
    try:
        cursor = conn.cursor()
        insert_query = """
            INSERT INTO AROrderDetails_1 (
                representative_code,
                client_code,
                client_name,
                order_date,
        ...
        cursor.executemany(insert_query, data)
        conn.commit()
        logging.info(f"MySQL AROrderDetails_1 테이블에 {cursor.rowcount}개의 레코드 삽입 완료.")
        cursor.close()
    except mysql.connector.Error as err:
        logging.error(f"MySQL AROrderDetails_1 삽입 오류: {err}")
        conn.rollback()
        sys.exit(1)
...
def etl_process():
    try:
        ...
        try:
            ...
            formatted_date_str = input_date.strftime('%Y%m%d')  # '20241227' 형식으로 변환
            logging.info(f"유효한 날짜 형식: {formatted_date_str}")
        except ValueError:
        ...
        # Step 3 데이터 추출 쿼리 (작동되는 정상 쿼리 참조)
        query_step3 = """
            SELECT date, 
                   full_name, 
                   rechain_no, 
        ...
        # 데이터 추출
        df_step3 = extract_data(conn, query_step3, params=(formatted_date_str,))
        ...
        # 데이터 엑셀로 저장
        try:
            step3_excel_path = os.path.join(OUTPUT_FOLDER, f'Step3_Data_{formatted_date_str}.xlsx')
        ...
        try:
            ...
            if not df_step3.empty:
                insert_mysql_ARTransactionsLedger(mysql_conn, df_step3)
            else:
                logging.warning("ARTransactionsLedger에 삽입할 데이터가 없습니다.")

            # AROrderDetailsItem 테이블에 삽입
            if not df_step3.empty:
                insert_mysql_AROrderDetailsItem(mysql_conn, df_step3)
            else:
                logging.warning("AROrderDetailsItem에 삽입할 데이터가 없습니다.")

            # AROrderDetails_1 테이블에 삽입
            if not df_step3.empty:
                insert_mysql_AROrderDetails_1(mysql_conn, df_step3)
            else:
                logging.warning("AROrderDetails_1에 삽입할 데이터가 없습니다.")
        except Exception as e:
