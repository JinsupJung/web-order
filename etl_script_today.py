...
def insert_mysql_ARTransactionsLedger(conn, df):
    ...
    try:
        ...
        data = [
            (
                row['date'],  # 'date' 컬럼을 'transaction_date' 필드에 매핑
                ' ',  # representative_code에 공백 삽입
                row['client'],  # 'rechain_no' 대신 'client' 사용
                row['outlet_name'],
                row['debit'],
    ...
    try:
        ...
        data = [
            (
                ' ',  # representative_code에 공백 삽입
                row['client_code'],
                row['client_name'],  # convert_to_utf8 적용
                ' ',  # collector_key에 공백 삽입
                ' ',  # manager에 공백 삽입
                row['order_date'],
                row['order_amount']
            )
    ...
    try:
        ...
        try:
            if not df_step3.empty:
                # ARTransactionsLedger: 그룹화 및 삽입 (수정 금지)
                # 이미 성공적으로 작동하고 있으므로 이 부분은 수정하지 않습니다.
                ...
                # **추가: client와 outlet_name 필드의 트림 처리**
                ...
                df_AROrderDetails['client_name'] = df_AROrderDetails['client_name'].apply(convert_to_utf8)  # UTF-8 변환
                ...
                df_AROrderDetailsItem['order_date'] = pd.to_datetime(
                    df_AROrderDetailsItem['order_date'], 
                    format='%Y%m%d', 
                    errors='coerce'  # 변환 실패 시 NaT로 설정
                ).dt.strftime('%Y-%m-%d')

                df_AROrderDetailsItem['tax'] = df_AROrderDetailsItem['tax'].apply(
                    lambda x: '과세' if x == 'tax   ' else ('면세' if x == 'no tax' else x)
                )
                ...
                # total을 total_amount로 이름 변경
                df_AROrderDetailsItem.rename(columns={
                    'total': 'total_amount'
                }, inplace=True)

                # order_date를 문자열로 변환 (이미 varchar로 설정되어 있으므로 형변환 필요 없음)
                df_AROrderDetailsItem['order_date'] = df_AROrderDetailsItem['order_date'].astype(str)

                # 데이터프레임의 컬럼명 확인 (디버깅 용도)
